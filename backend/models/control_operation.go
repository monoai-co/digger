...
type ExecutionClaimAttempt struct {
	...
	SigningKeyID        string              `gorm:"type:text;not null;index:idx_execution_claim_unexpired_keys,priority:2,where:state = 'granted';check:execution_claim_attempts_signing_key_check,length(signing_key_id) BETWEEN 1 AND 128 AND trim(signing_key_id) = signing_key_id"`
	...
	GrantExpiresAt      time.Time           `gorm:"not null;index:idx_execution_claim_unexpired_keys,priority:1,where:state = 'granted';check:execution_claim_attempts_grant_expiry_check,grant_expires_at > created_at AND (granted_at IS NULL OR grant_expires_at > granted_at)"`
